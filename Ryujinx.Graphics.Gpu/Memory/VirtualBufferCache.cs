    ...
    class VirtualBufferCache
    {
        ...
        private void CreateBufferAligned(ulong gpuVa, ulong size)
        {
            ...
            if (overlapsCount != 0)
            {
                ...
                if (_viewOverlaps[0].Address <= gpuVa && _viewOverlaps[0].EndAddress >= endAddress)
                {
                    return;
                }
                    ...
                    endAddress - _viewOverlaps[0].EndAddress <= BufferAlignmentSize * 2)
                {
                    ...
                    ulong existingSize = _viewOverlaps[0].Size;
        ...
        private Buffer GetBuffer(ulong gpuVa, ulong size, bool write, out int bufferOffset, out MultiRange range)
        {
            ...
            if (size != 0)
            {
                ...
                if (buffer != null)
                {
                    ...
                    /* if (range.GetSubRange(0).Address != memoryManager.Translate(gpuVa))
                    {
                        throw new Exception($"addr mismatch 0x{range.GetSubRange(0).Address:X} vs 0x{memoryManager.Translate(gpuVa):X}");
                    } */
                ...
                {
                    throw new Exception($"NOT FOUND 0x{gpuVa:X} 0x{size:X}");
                }
            }
