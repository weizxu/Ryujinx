...
namespace Ryujinx.Graphics.Gpu.Image
{
    ...
    class TextureBindingsManager : IDisposable
    {
        ...
        private void CommitTextureBindings(TexturePool pool, ShaderStage stage, int stageIndex)
        {
            ...
            for (int index = 0; index < textureCount; index++)
            {
                ...
                if (hostTexture != null && texture.Target == Target.TextureBuffer)
                {
                    ...
                    _channel.BufferManager.SetBufferTextureStorage(hostTexture, texture.Range, bindingInfo, bindingInfo.Format, false);
                }
                else
                {
        ...
        private void CommitImageBindings(TexturePool pool, ShaderStage stage, int stageIndex)
        {
            ...
            for (int index = 0; index < imageCount; index++)
            {
                ...
                if (hostTexture != null && texture.Target == Target.TextureBuffer)
                {
                    ...
                    _channel.BufferManager.SetBufferTextureStorage(hostTexture, texture.Range, bindingInfo, format, true);
                }
                else
                {
                    if (isStore)
            ...
            int cbufSlot)
        {
            ...
            // System.Console.WriteLine($"get tex ID 0x{textureId:X} packed 0x{packedId:X} handle 0x{handle:X} slots 0x{cbufSlot:X}");

            texturePool.TryGetDescriptor(textureId, out TextureDescriptor descriptor);
            return descriptor;
        }
