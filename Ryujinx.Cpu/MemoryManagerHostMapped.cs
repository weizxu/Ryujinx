...
namespace Ryujinx.Cpu
{
    ...
    public class MemoryManagerHostMapped : MemoryManagerBase, IMemoryManager, IVirtualMemoryManagerTracked, IWritableBlock
    {
        ...
        private class Mapping : IRange
        {
            ...
            public Mapping(ulong virtualAddress, ulong size, ulong physicalAddress)
            {
        ...
        public MemoryManagerHostMapped(MemoryBlock backingMemory, ulong addressSpaceSize, bool unsafeMode, InvalidAccessHandler invalidAccessHandler = null)
        {
            ...
            _addressSpaceMirror = new MemoryBlock(asSize, MemoryAllocationFlags.Reserve | MemoryAllocationFlags.ViewCompatible);
        ...
        public void Map(ulong va, ulong pa, ulong size)
        {
            ...
            AddMapping(va, size);
            AddMappingToRangeList(va, pa, size);
        ...
        public void Unmap(ulong va, ulong size)
        {
            ...
            RemoveMappingFromRangeList(va, size);
        ...
        public IEnumerable<MemoryRange> GetPhysicalRegions(ulong va, ulong size)
        {
            ...
            lock (_mappings)
            {
                count = _mappings.FindOverlaps(new Mapping(va, size, 0UL), ref overlaps);
            }
            ...
            for (int index = 0; index < count; index++)
            {
                ...
                ulong overlapVa = overlap.Address;
                ulong overlapEndVa = overlap.EndAddress;
                ...
                if (overlapEndVa > endAddress)
                {
                    overlapEndVa = endAddress;
                }

                ulong offset = overlapVa - overlap.Address;

                regions.Add(new MemoryRange(overlap.PhysicalAddress + offset, overlapEndVa - overlapVa));
            }
        ...
        private void AddMappingToRangeList(ulong va, ulong pa, ulong size)
        {
            lock (_mappings)
            {
                RemoveMappingFromRangeList(va, size);
                _mappings.Add(new Mapping(va, size, pa));
            }
        }

        private void RemoveMappingFromRangeList(ulong va, ulong size)
        {
            ulong endAddress = va + size;
            Mapping[] overlaps = Array.Empty<Mapping>();
            ...
            lock (_mappings)
            {
                int count = _mappings.FindOverlaps(new Mapping(va, size, 0UL), ref overlaps);
                ...
                for (int index = 0; index < count; index++)
                {
                    ...
                    {
                        _mappings.Add(new Mapping(overlap.Address, va - overlap.Address, overlap.PhysicalAddress));
                    }

                    if (overlap.EndAddress > endAddress)
                    {
                        _mappings.Add(new Mapping(endAddress, overlap.EndAddress - endAddress, overlap.PhysicalAddress + (endAddress - overlap.Address)));
                    }
                }
            }
        }
